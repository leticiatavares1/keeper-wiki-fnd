...
**O build precisa da API de `../keeper-wiki-bkd` no ar.** O dado do jogo é buscado no prerender, não no navegador: sem API, o build falha de propósito, com a mensagem dizendo o que subir. O site gerado continua 100% estático e não fala com a API em runtime.
...
- As listas que quase toda página usa (`/itens`, `/estacoes`, `/tecnologias`) são buscadas **uma vez por build** e memorizadas: são ~1.400 páginas no mesmo processo.
...
| `/[game]/itens/[id]` | ficha do item: o que o faz e o que o gasta (1.157 páginas) |
...
A ficha existe para **todo** item, inclusive os marcados como `nao_usado`: oito deles ainda aparecem em receita, e link quebrado derruba o prerender. O índice de itens esconde esses, atrás de uma caixa de seleção.
