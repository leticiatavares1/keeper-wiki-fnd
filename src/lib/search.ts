import type { ItemCard, RecipeCard, Station, Tech } from '$lib/api/types';
import { itemName, refName, stationName, techBranch, techName } from './format';
...
export function filterItems<T extends ItemCard>(items: T[], query: string, tipo = ''): T[] {
	return items.filter((i) => (!tipo || i.tipo === tipo) && matches(query, [i.pt, i.en, i.id]));
}
...
export function filterStations(stations: Station[], query: string): Station[] {
	return stations.filter((e) => matches(query, [e.pt, e.en, e.id]));
}

/** Busca pelo nome da tecnologia e pelo que ela libera. */
export function filterTechs(techs: Tech[], query: string, branch = ''): Tech[] {
	return techs.filter(
		(t) =>
